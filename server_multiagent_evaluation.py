...
def decode_action(index, action_dict, num_cameras):
    ...
    action_str = global_action_dic[int(index)]
...
def make_env(debug_mode=False):
    ...
    return env
def evaluate_trained_model(model_path, num_episodes=1, debug_mode=False):
    ...
    for episode in range(num_episodes):
        ...
        print(f"[Episode {episode+1}] Global TP Reward = {episode_reward:.4f} in {step} steps")
        all_rewards.append(episode_reward)

    avg_reward = sum(all_rewards) / len(all_rewards)
    print(f"\n✅ Evaluation Finished. Average TP Reward over {num_episodes} episodes: {avg_reward:.4f}")
    
    env.close()


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Unity ML-Agents Script")
    parser.add_argument("--mode", type=str, choices=["train", "eval"], default="train", help="Mode: train or eval")
    parser.add_argument("--model_path", type=str, default="", help="Path to the trained model (only for eval)")
    ...
    if args.mode == "train":
        ...
        train_env = make_env(debug_mode=args.debug)
        ...
        if not args.model_path:
            print("❌ Please provide --model_path to the trained model (without .zip)")
        else:
            evaluate_trained_model(model_path=args.model_path, num_episodes=1, debug_mode=args.debug)
